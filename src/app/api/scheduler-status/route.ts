...
export async function GET() {
  return NextResponse.json({
    ...
    cron_schedule: {
      description: "Vercel cron jobs run automatically at 05:00 UTC daily",
      schedule: "0 5 * * * (UTC time)",
      troop_selection: "/api/dailyTroopSelection",
      map_area_selection: "/api/dailyMapAreaSelection",
      next_run: "Today at 05:00 UTC (in about " + getTimeUntilNextRun() + ")"
    },
...
function getTimeUntilNextRun(): string {
  ...
  // If it's already past 05:00 today, set for tomorrow
  if (now >= nextRun) {
